...
### 2. Build the MCP Gateway

The gateway source lives in a sibling directory:
...
MINIO_BUCKET=radiology-images
EOF

echo "GATEWAY_SIGNING_KEY=$(openssl rand -hex 32)" >> .env
```

### 5. Load sample X-rays into MinIO
...
└── .env                           # MinIO credentials + gateway signing key (not committed)
```
